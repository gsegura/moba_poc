...
class MoBAConfig:
    ...
    moba_chunk_size: int  # Size of each chunk for block attention
    moba_topk: int  # Number of top chunks to attend to

@lru_cache(maxsize=16)
def calc_chunks(cu_seqlen: torch.Tensor, moba_chunk_size: int):
    """Calculate chunks that need MoBA attention.
    
    Args:
        cu_seqlen: Cumulative sequence lengths tensor
        moba_chunk_size: Size of each chunk/block
        
    Returns:
        Tuple containing:
        - cu_chunk: Cumulative chunk offsets
        - filtered_chunk_indices: Indices of chunks needing MoBA attention
        - num_filtered_chunk: Number of filtered chunks
        - chunk_to_batch: Mapping from chunk indices to batch indices
    """
    # Calculate batch sizes and number of chunks per batch
    batch_sizes = cu_seqlen[1:] - cu_seqlen[:-1]
    batch_num_chunk = (batch_sizes + (moba_chunk_size - 1)) // moba_chunk_size
    
    # Calculate cumulative number of chunks
    ...
    chunk_sizes[0] = 0  # For calculating cumulative chunks
    batch_last_chunk_size = batch_sizes - (batch_num_chunk - 1) * moba_chunk_size
    chunk_sizes[cu_num_chunk[1:]] = batch_last_chunk_size
    
    # Calculate cumulative chunks and chunk-to-batch mapping
    cu_chunk = chunk_sizes.cumsum(dim=-1, dtype=torch.int32)
    chunk_to_batch = torch.zeros((num_chunk,), dtype=torch.int32, device=cu_seqlen.device)
    chunk_to_batch[cu_num_chunk[1:-1]] = 1
    chunk_to_batch = chunk_to_batch.cumsum(dim=0, dtype=torch.int32)

    # Filter out last chunk of each batch
...
class MixedAttention(torch.autograd.Function):
    """Mixed attention combining self-attention and MoBA attention."""
    ...
    def forward(ctx, q, k, v, self_attn_cu_seqlen, moba_q, moba_kv, 
        ...
        # Ensure tensors are on the CUDA device and in fp16
        q, k, v = q.cuda().half(), k.cuda().half(), v.cuda().half()
        self_attn_cu_seqlen = self_attn_cu_seqlen.cuda()  # int tensor remains as-is
        moba_q, moba_kv = moba_q.cuda().half(), moba_kv.cuda().half()
...
def moba_attn_varlen(
    ...
    # Ensure tensors are on CUDA and in fp16 for FlashAttention ops
    q, k, v = q.cuda().half(), k.cuda().half(), v.cuda().half()
    ...
    Args:
        q: Query tensor [seqlen, head, head_dim]
        k: Key tensor [seqlen, head, head_dim]
        v: Value tensor [seqlen, head, head_dim]
        cu_seqlens: Cumulative sequence lengths tensor
        max_seqlen: Maximum sequence length in batch
        moba_chunk_size: Size of each chunk/block
        moba_topk: Number of top chunks to attend to
        
    Returns:
        Attention output tensor [seqlen, head, head_dim]
    """
